local CoinLeaderboard = {} -- TemplateCoin
local DSS = game:GetService("DataStoreService")
local Players = game:GetService("Players")
local LeaderboardFolder = workspace.Leaderboard.Coin
local DataSave = require(script.Parent.Parent.Server.Data)
local LeaderStore = DSS:GetOrderedDataStore("TestAlifa135144")
...
local Holder = LeaderboardFolder.GuiTwoBoard.SurfaceGui:WaitForChild("Holder")
local Template = game.ReplicatedStorage.Assert.LeaderboadrTemp:WaitForChild("TemplateCoin")
...
task.spawn(function()
	while true do
		for _, Player in pairs(Players:GetPlayers()) do
			...
			local Currency = PData.BaseSettings.Coin
